...
public class UtilsText {
    ...
    public static String convert2CamelCase(String underScore) {
    	...
    	boolean isUnderScore = underScore.indexOf('_') > -1;
        // '_' 가 나타나지 않으면 이미 camel case 로 가정함.
        // 단 첫째문자가 대문자이면 camel case 변환 (전체를 소문자로) 처리가
        // 필요하다고 가정함. --> 아래 로직을 수행하면 바뀜
        if (!isUnderScore && Character.isLowerCase(underScore.charAt(0))) {
            return underScore;
        }
        ...
        for (int i = 0; i < len; i++) {
			if (!isLowerCase) {
				isLowerCase = Character.isLowerCase(underScore.charAt(i));
			}
     ...
     * @return Camel 표기법 변수명
     */
    public static String convert2CamelCaseTable(String underScore,boolean notCamelCase) {

        // '_' 가 나타나지 않으면 이미 camel case 로 가정함.
        // 단 첫째문자가 대문자이면 camel case 변환 (전체를 소문자로) 처리가
        // 필요하다고 가정함. --> 아래 로직을 수행하면 바뀜
        if (underScore.indexOf('_') < 0
            && Character.isLowerCase(underScore.charAt(0)) || notCamelCase == true) {
            return underScore;
        }
        StringBuilder result = new StringBuilder();
        boolean nextUpper = false;
    ...
    public static String convert2CamelCaseTable(String underScore) {
    	return convert2CamelCaseTable(underScore, false);
    }
	...
	*/
	...
	/**
	 * camelcase 문자에서 snakecase 문자로 변경.
	 *
	 * @param camelCase
	 * @return
	 *
	 * @since 2019.06.16 11:20
	 * @author jangcool
	 */
	public static String convertSnakeCaseToCamelcase(String camelCase) {
